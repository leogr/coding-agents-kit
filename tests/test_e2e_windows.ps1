...
.DESCRIPTION
    Starts Falco with the plugin and http_output, writes rules, sends
    test events via the interceptor, and validates verdicts.
...
$PluginDll = Find-Built (Join-Path $RootDir 'plugins\coding-agent-plugin') 'coding_agent.dll'
...
function Start-Falco {
    ...
    Copy-Item $PluginDll "$FalcoDir\coding_agent.dll" -Force -ErrorAction SilentlyContinue

    $denyRules = (Join-Path $RulesDir 'deny.yaml') -replace '\\', '/'
    $seenRules = (Join-Path $RulesDir 'seen.yaml') -replace '\\', '/'

    # Write YAML config with http_output (alerts delivered via curl to plugin HTTP server)
...
"@ | Set-Content $falcoConfig -Encoding UTF8
    ...
    # Launch Falco directly (http_output delivers alerts in-process)
    $psi = New-Object System.Diagnostics.ProcessStartInfo
    $psi.FileName = Join-Path $FalcoDir 'falco.exe'
    ...
    $script:falcoProcess.BeginOutputReadLine()
    $script:falcoProcess.BeginErrorReadLine()
